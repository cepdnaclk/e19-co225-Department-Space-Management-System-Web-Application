...
public interface WaitingRepository extends CrudRepository<Waiting, Long> {
    ...
    // Finds a list of Waiting entities by space ID, start date/time, and end
    // date/time.
    ...
    Waiting findBySpaceIDAndDateAndReservedByIdAndResponsiblePersonId(int spaceID, String date, long reservedById,
            long responsiblePersonId);

    Waiting findBySpaceIDAndStartDateTimeAndEndDateTimeAndAndReservedByIdAndResponsiblePersonId(int spaceID,
            Date startDateTime, Date endDateTime, long reservedById, long responsiblePersonId);
    ...
    // Finds a list of Waiting entities by reservation date/time before a given
    // date/time.
