...
public class WaitingController {
    ...
    // The @GetMapping annotation maps HTTP GET requests to the /waiting/slot endpoint
    @GetMapping("/slot")
    public List<WaitingResponse> getWaitingList(@RequestParam int spaceID, @RequestParam String date,
        ...
        // Create a new Slot object with the provided parameters and call the getWaitingList() method of the WaitingService to return a List of WaitingResponse objects
        Slot slot = new Slot(spaceID, date, startTime, endTime);
        return waitingService.getWaitingList(slot);
    }

    // The @GetMapping annotation maps HTTP GET requests to the /waiting/user endpoint
    @GetMapping("/user")
    public List<ReservationResponse> getUserWaitingList(@RequestParam String email) throws ResponseStatusException {

        // Check if the email in the request parameters matches the email in the SecurityContext (commented out)
        // If not, return an empty ArrayList (commented out)

        try {
            // Call the getUserWaitingList() method of the WaitingService with the provided email and return a List of ReservationResponse objects
            return waitingService.getUserWaitingList(email);
        } catch (Exception e) {
            // If an exception is caught, throw a new ResponseStatusException with an HTTP status of 400 and an error message
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "invalid\n");
        }
    }

    // The @GetMapping annotation maps HTTP GET requests to the /waiting/responsible endpoint
    ...
    public List<ReservationResponse> getResponsibleWaitingList(@RequestParam String email)
        ...
        try {
            // Call the getResponsibleWaitingList() method of the WaitingService with the provided email and return a List of ReservationResponse objects
    ...
    public ReservationResponse createWaiting(ReservationRequest waiting) throws ResponseStatusException {
        ...
        try {
            ...
            // If an AllReadyWaitingException is caught, throw a new ResponseStatusException with an HTTP status of 400 and an error message
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "invalid\n");
        }

    }

    // The @DeleteMapping annotation maps HTTP DELETE requests to the /waiting endpoint
    @DeleteMapping()
    public String deleteWaiting(@RequestParam int spaceID, @RequestParam String date, @RequestParam int startTime,
        ...
        try {
            // Create a new Slot object with the provided parameters and call the waitingDeleteBySlot() method of the WaitingService with the email and return a String message
            Slot slot = new Slot(spaceID, date, startTime, endTime);
            return waitingService.waitingDeleteBySlot(slot, email);
        } catch (InvalidDataException e) {
            // If an InvalidDataException is caught, throw a new ResponseStatusException with an HTTP status of 400 and an error message
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "invalid\n");
        }
